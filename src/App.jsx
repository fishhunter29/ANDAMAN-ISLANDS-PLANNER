  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // --- Normalize LOCATIONS: location→name, typicalHours→durationHrs, plus slug ---
        ...
        // --- Normalize ACTIVITIES (no structural change needed, keep as-is) ---
        const acts = Array.isArray(actsRaw) ? actsRaw : [];

        // --- Ferries: keep as-is, but ensure array ---
        const fers = Array.isArray(fersRaw) ? fersRaw : [];

        // --- Location→Adventures map (allow id OR slug) ---
        const mapArr = Array.isArray(mapRaw) ? mapRaw : [];

        setLocations(normalizedLocs);
        setActivities(acts);
        setFerries(fers);
        setLocAdventures(mapArr);
        ...
        <section>
          ...
          {step === 0 && (
            <Card title="Trip Basics">
              ...
              <Row>
                <label>
                  <input type="checkbox" checked={startPB} onChange={() => setStartPB(!startPB)} /> Start from Port
                  Blair if present
                </label>
              </Row>
                        ...
                        {day.items.map((it, k) => (
                          ...
                          >
                            ...
                            <span style={{ display: "inline-flex", gap: 6 }}>
                              <button onClick={() => moveItem(i, k, -1)} style={miniBtn} title="Move to previous day">
                                ◀︎
                              </button>
                              <button onClick={() => moveItem(i, k, +1)} style={miniBtn} title="Move to next day">
                                ▶︎
                              </button>
                            </span>
                          </li>
                      ...
                      {!day.items.some((it) => it.type === "ferry") &&
                        !day.items.some((i) => i.type === "departure") && (
                          ...
                          >
                            ...
                            <button onClick={() => addEmptyDayAfter(i)} style={pillBtn}>
                              + Add empty day after
                            </button>
                            <button onClick={() => deleteDay(i)} style={dangerBtn} disabled={days.length <= 1}>
                              Delete day
                            </button>
                          </div>
                        )}
                  ...
                  <Row>
                    <Field label="Class">
                      <select
                        ...
                        onChange={(e) => setEssentials({ ...essentials, ferryClass: e.target.value })}
                      >
                  ...
                  <Row>
                    <Field label="Cab model (Day Cab days)">
                      <select
                        ...
                        onChange={(e) => setEssentials({ ...essentials, cabModelId: e.target.value })}
                      >
                        {CAB_MODELS.map((c) => (
            ...
            >
              ...
              <div style={{ padding: 16 }}>
                <div style={{ fontSize: 14, color: "#334155", display: "grid", gap: 4 }}>
                  ...
                  <div>
                    Travellers: <b>{adults} adult(s){infants ? `, ${infants} infant(s)` : ""}</b>
                  </div>
                </div>
            ...
            <div style={{ padding: 14 }}>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {(() => {
                    // match by id OR slug
                    const byId = locAdventures.find(
                      (m) => String(m.locationId).toLowerCase() === String(openLoc.id).toLowerCase()
                    );
                    const bySlug = locAdventures.find(
                      (m) => String(m.locationId).toLowerCase() === String(openLoc.slug).toLowerCase()
                    );
                    const ids = [...(byId?.adventureIds || []), ...(bySlug?.adventureIds || [])];
                    const uniq = Array.from(new Set(ids));
                    const top = activities.filter((a) => uniq.includes(a.id)).slice(0, 3);
                    const show =
                      top.length > 0
                        ? top
                        : activities
