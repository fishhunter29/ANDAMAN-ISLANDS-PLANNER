...
function ferryDay(fromIsland,toIsland,f){
  const label = f ? `Ferry ${f.from} → ${f.to} (${f.operator} ${f.depart}–${f.arrive}, ${f.duration_label})`
                  : `Ferry ${fromIsland} → ${toIsland}`;
  return { island: fromIsland, transport:"—", windows:[{key:"ferry",label:"Ferry",cap:24,used:0,items:[{type:"ferry",name:label}]}] };
}
...
export default function App(){
  ...
  useEffect(()=>{
    Promise.all([
      ...
      setLocations(locs); setActivities(acts); setFerries(fers);
  ...
  const [selectedIds,setSelectedIds]   = useState(["pb_airport"]); // Airport preselected
  ...
  const filtered = useMemo(()=>{
    let list = islandFilter==="All" ? locations : locations.filter(l=> (l.island||"").toLowerCase()===islandFilter.toLowerCase());
    if (offbeat) list = list.filter(l => (l.tags||[]).includes("offbeat") || (l.tags||[]).includes("less-crowded"));
  ...
  const islandSeq = useMemo(()=>{
    const seq = [];
    locations.filter(l=>selectedIds.includes(l.id)).forEach(l=>{ if(l.island && !seq.includes(l.island)) seq.push(l.island); });
    return seq;
  ...
  const caps = useMemo(()=>{
    ...
    if (mood==="fast")    return {morning:3.5,afternoon:4.5,evening:3,max:6};
  ...
  const days = useMemo(()=>{
    ...
    islandSeq.forEach(island=>{
      ...
      locs.forEach(l=>{
        ...
        // try place in existing day
        let placed=false;
        for(const day of d){
          if(day.transport==="—" || day.island!==island) continue;
          const itemsCount = day.windows.reduce((a,w)=>a+w.items.filter(i=>i.type==="location").length,0);
  ...
  const logistics = useMemo(()=>{
    ...
    days.forEach(day=>{
      ...
      const stops = day.windows.reduce((a,w)=>a+w.items.filter(i=>i.type==="location").length,0);
  ...
  const counts = useMemo(()=>({
    ...
    essentials:0, hotels:0, addons: addonIds.length
    ...
    <div>
      ...
      <header style={{position:"sticky",top:0,zIndex:30,background:"white",borderBottom:"1px solid #e5e7eb"}}>
        <div className="container" style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px"}}>
          ...
          <span style={{marginLeft:"auto",fontSize:12,color:"#475569"}}>Selected: {selectedIds.length} • Days: {days.length}</span>
        </div>
      </header>
      ...
      <main className="container" style={{display:"grid",gridTemplateColumns:"1fr 360px",gap:16}}>
        <section>
          {/* Islands */}
          <section id="panel-islands" role="tabpanel" hidden={step!=="islands"} aria-labelledby="tab-islands">
            <div className="card">
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                {ISLANDS.map(i=> <button key={i} onClick={()=>setIslandFilter(i)} style={chip(islandFilter===i)}>{i}</button>)}
              </div>
              <div style={{fontSize:12,color:"#64748b",marginTop:8}}>Tip: Switching islands will add a ferry leg later.</div>
            </div>
          </section>

          {/* Mood */}
          <section id="panel-mood" role="tabpanel" hidden={step!=="mood"} aria-labelledby="tab-mood">
            <div className="card">
              <div style={{fontWeight:700,marginBottom:10}}>Trip mood</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <button onClick={()=>setMood("relaxed")}  style={chip(mood==="relaxed")}>Relaxed</button>
                <button onClick={()=>setMood("balanced")} style={chip(mood==="balanced")}>Balanced</button>
                <button onClick={()=>setMood("fast")}     style={chip(mood==="fast")}>Fast-paced</button>
              </div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>
                <button onClick={()=>setOffbeat(v=>!v)} style={chip(offbeat)}>Off-beat</button>
                <button onClick={()=>setFamily(v=>!v)}  style={chip(family)}>Family-friendly</button>
              </div>
            </div>
          </section>

          {/* Locations */}
          <section id="panel-locations" role="tabpanel" hidden={step!=="locations"} aria-labelledby="tab-locations">
            <div className="card">
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
                {filtered.map(l=>{
                  ...
                  const advs = activities.filter(a=> (a.special_locations||[]).some(p=>norm(p).includes(norm(l.name)) || norm(p).includes(norm(l.island)) ));
                  return (
                    <div key={l.id} className="card" style={{padding:12}}>
                      ...
                      <div style={{fontSize:12,color:"#64748b",marginTop:4}}>{l.island} • Best: {l.best_time||"—"} • {l.duration_hrs||2}h</div>
                      <div style={{marginTop:8}}>
                        <label style={{fontSize:12,color:"#475569"}}>Add adventure (optional)</label>
                        <select defaultValue="" onChange={(e)=>{ const id=e.target.value; if(id && !addonIds.includes(id)) setAddonIds(prev=>[...prev,id]); e.target.value=""; }} style={{width:"100%",marginTop:4}}>
                          <option value="">— Choose —</option>
                          {advs.map(a=>(<option key={a.id} value={a.id}>{a.activity} ({formatINR(a.approx_cost_inr)})</option>))}
                        </select>
                      </div>
                      <button onClick={()=> setSelectedIds(prev => prev.includes(l.id) ? prev.filter(x=>x!==l.id) : [...prev,l.id])}
                        style={{marginTop:10,width:"100%",padding:"8px 10px",borderRadius:8,border:"1px solid var(--accent)",
                                background:selected?"var(--accent)":"white",color:selected?"white":"var(--accent)",fontWeight:700}}>
          ...
          {/* Itinerary */}
          <section id="panel-itinerary" role="tabpanel" hidden={step!=="itinerary"} aria-labelledby="tab-itinerary">
            <div className="card">
